...
# fzf-tab previews (eza for directories, bat for files)
zstyle ':fzf-tab:complete:*:*' fzf-preview '
  if [[ -d ${(Q)realpath} ]]; then
    eza -1 --color=always -- ${(Q)realpath}
  elif [[ -f ${(Q)realpath} ]]; then
    bat --style=numbers --color=always --line-range=:100 -- ${(Q)realpath} 2>/dev/null || cat ${(Q)realpath}
  fi
'
